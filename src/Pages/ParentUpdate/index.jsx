...
export default function ParentUpdate({
  ...
  const getParent = async () => {
    ...
    setParentForm({
      ...
      primary: {
        ...ContextConsumer?.Profile?.details?.primary,
        image: undefined,
      },
      spouse: {
        ...ContextConsumer?.Profile?.details?.spouse,

        image: undefined,
      },
    });
    // setParentForm(r.data.data[0]);
    setInitialPrimaryImage(ContextConsumer?.Profile?.details?.primary?.image);
    setInitialSpouseImage(ContextConsumer?.Profile?.details?.spouse?.image);
  };
  useEffect(() => {
    getParent();
  }, []);
